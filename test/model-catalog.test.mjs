...
const cache=JSON.stringify({identity:'private',models:[
  ...
  {slug:'jev-router',display_name:'Legacy Jev Router',visibility:'list',supported_in_api:true,priority:0},
...
test('catalog places Jev first and keeps usable cached models',()=>{
  ...
  assert.deepEqual(catalog.models.map(model=>model.slug),['gpt-reserve','gpt-5.6-sol','gpt-5.6-terra']);
...
test('catalog can match the models available to the signed-in account',()=>{
  ...
  assert.deepEqual(catalog.models.map(model=>model.slug),['gpt-reserve','gpt-5.6-terra']);
...
test('catalog text is stable JSON with a trailing newline',()=>{
  ...
  assert.equal(JSON.parse(text).models[0].slug,'gpt-reserve');
});

test('catalog refuses to create an unsupported custom model identifier',()=>{
  assert.throws(()=>buildModelCatalog(JSON.stringify({models:[{slug:'gpt-5.6-sol',visibility:'list'}]})),/carrier/);
});
