...
import { resolveBackend, fallbackCatalog } from './backend.mjs';
...
export async function startServer({ port = DEFAULT_PORT, upstream = UPSTREAM, backend, store = new StateStore(), route,
  defaultModel = 'gpt-5.6-sol', event = () => {}, deadlineMs = 8000, mode = 'active', shadowBaseline = null } = {}) {
  backend ??= { name: 'openai-http', baseUrl: upstream, token: null, subscription: true };
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/healthz') return reply(res, 200, { ok: true, version: '0.1.0', pid: process.pid, startedAt, requests, backend: backend.name, mode: router.mode, shadowBaseline: router.shadowBaseline, deadlineMs: router.deadlineMs });
      ...
      const incomingAuth = req.headers.authorization?.startsWith('Bearer ') ? req.headers.authorization : null;
      const authorization = backend.token ? `Bearer ${backend.token}` : incomingAuth;
      if (!authorization || (backend.subscription && !req.headers['chatgpt-account-id'])) return reply(res, 401, { error: 'provider_auth_required' });
      requests++;
      const account = digest(req.headers['chatgpt-account-id'] ?? backend.name);
      const headers = { ...safeHeaders(req.headers), authorization, 'accept-encoding': 'identity' };
      if (!backend.subscription) delete headers['chatgpt-account-id'];
      ...
      const fetchCatalog = async () => {
        ...
        const response = await upstreamRequest(`${backend.baseUrl}/models${query}`, { method: 'GET', headers }, null, abort.signal);
        const data = await readBody(response, 4 * 1024 * 1024);
        if (response.statusCode !== 200) {
          if (!backend.subscription) return fallbackCatalog();
          throw Object.assign(new Error('Model catalog request failed'), { status: response.statusCode });
        }
        let parsed;
        ...
        if (!Array.isArray(parsed.models) || !parsed.models.length) {
          if (!backend.subscription) return fallbackCatalog();
          throw Object.assign(new Error('Invalid model catalog'), { status: 502 });
        }
      ...
      const response = await upstreamRequest(`${backend.baseUrl}${url.pathname}${url.search}`, { method: 'POST', headers }, Buffer.from(JSON.stringify(body)), abort.signal);
...
export async function main() {
  ...
  if (existsSync(envFile)) {
    ...
    // routing tier overrides (JEV_CODEX_*_MODEL) plus the catalog pin
    // (JEV_DESKTOP_FALLBACK_MODELS) have to reach the process that actually serves requests.
  ...
  const instance = await startServer({ port: Number(process.env.JEV_DESKTOP_PORT ?? DEFAULT_PORT), backend: resolveBackend(), store: new StateStore(join(directory, 'state.json')),
